...
class Network:
    ...
    def construct(self, args, num_words, num_chars, num_tags, num_subtags):
        ...
        with self.session.graph.as_default():
            ...
            self.tags = tf.placeholder(tf.int32, [None, None, morpho_dataset.MorphoDataset.SUBTAG_COUNT], name="tags")
            ...
            output_layers = []
            for i in range(morpho_dataset.MorphoDataset.SUBTAG_COUNT):
                output_layers.append(tf.layers.dense(rnn_output, num_subtags[i], activation=None, name="output_layer_" + str(i)))
            ...
            self.predictions = tf.add_n([
                tf.scalar_mul(10**i, tf.argmax(output_layer, axis=2))
            ...
            tags_separated = [tf.gather(self.tags, [i], axis=2) for i in range(morpho_dataset.MorphoDataset.SUBTAG_COUNT)]
            tags = tf.add_n(
                [tf.scalar_mul(10**i, tf.gather(self.tags, [i], axis=2)) for i in range(morpho_dataset.MorphoDataset.SUBTAG_COUNT)]
            )
            losses = [tf.losses.sparse_softmax_cross_entropy(t, l, weights=weights) for l, t in zip(output_layers, tags_separated)]
            ...
            self.current_accuracy, self.update_accuracy = tf.metrics.accuracy(tags, self.predictions, weights=weights)
...
if __name__ == "__main__":
    ...
    dev = morpho_dataset.MorphoDataset("czech-pdt-dev.txt", train=train, shuffle_batches=False, subtags=train._factors[train.TAGS])
    test = morpho_dataset.MorphoDataset("czech-pdt-test.txt", train=train, shuffle_batches=False, subtags=dev._factors[dev.TAGS])
    ...
    network.construct(args, len(train.factors[train.FORMS].words), len(train.factors[train.FORMS].alphabet),
                      ...
                      [len(train._factors[train.TAGS].subtags[i]) for i in range(morpho_dataset.MorphoDataset.SUBTAG_COUNT)]
    )
    print([len(train._factors[train.TAGS].subtags[i]) for i in range(morpho_dataset.MorphoDataset.SUBTAG_COUNT)])
